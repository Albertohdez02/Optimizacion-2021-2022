...
typedef struct {
	unsigned extremo1, extremo2;		//extremos de la arista
	int peso;
} AristaPesada;
...
public:
     ...
     void RecorridoAmplitud(); //Construye un recorrido en amplitud desde un nodo inicial
		 void Kruskal();  // Metodo que halla el arbol de menor coste por Kruskal
		 void MostrarCamino(unsigned s, unsigned i, vector<unsigned> pred); //metodo recursivo
		 void Dijkstra(); // Metodo que halla los caminos minimos a un nodo por Dijkstra
